...
public class Ex10_plactice_3 {
		static int paycheckCount(Calendar from, Calendar to) {
			//1.from 또는 to가 null이면 0을 반환한다
			
			if(from == null || to==null) return 0 ;
			
			//2. from 과 to가 같고 날짜가 21일이면 1을 반환하다.
			...
			//5.만일 from의 일(DAY_OF_MONTH)이 21일 이거나 이전이고 to의 일(")이 21일 이거나 이후이면,
			// monDiff의 값을 1 증가 시킨다.
			if(fromDay <= 21 && toDay >= 21)
				monDiff++;
			
			//6.만일 from의 일(DAY_OF_MONTH)이 21일 이후고 to의 일(")이 21일 이전이면,
			// monDiff의 값을 1 감소 시킨다. 
			if(fromDay < 21 && toDay <21)
				monDiff--;
